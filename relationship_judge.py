...
- 好感<70: 只能是陌生人或网友（日常闲聊不改变标签）
- 好感70-80: 可以升级到普通朋友，但需要对方有明确的友好表示
- 好感80-90: 可以是好朋友，但暧昧/恋人必须对方先表态
- 好感90+: 只有对方明确表白且你接受了才能是恋人
- 关系标签一旦确定不会轻易变化
- 重要：标签升级必须有明确的对话内容支持，不能仅靠好感度数值"""
        elif self.openness == "open":
            label_instruction = f"""你是开放型性格。关系标签变化规则：
- 好感<65: 陌生人或网友
- 好感65-75: 自然过渡到普通朋友（需要多次正面互动才能升级，仅闲聊不够）
- 好感75-85: 如果对话中有暧昧信号（表白、暗示、频繁示好），标签可以是暧昧关系
- 好感85+: 心里已经把对方当很亲近的人了
- 标签随心情和互动质量流动，但不能因为日常闲聊就跳到暧昧
- 重要：标签升级需要对话内容支持，不能仅因好感度数值就改变"""
        else:  # chaotic
